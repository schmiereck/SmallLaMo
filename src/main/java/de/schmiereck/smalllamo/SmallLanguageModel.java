 ...
 * Das Modell verwendet ein LSTM-Netzwerk mit internem Kontext-Gedächtnis, um bei gegebener
 * Eingabe (ein Zeichen) das nächste Zeichen vorherzusagen.
 */
public class SmallLanguageModel {
    ...
    private final int hiddenLayerSize = 32; // Größe der versteckten Schicht
     ...
     * und LSTM-Schichten für Kontextverarbeitung.
     */
    public SmallLanguageModel() {
        ...
        MultiLayerConfiguration config = new NeuralNetConfiguration.Builder()
                ...
                .list()
                // Erste versteckte LSTM-Schicht als Vorverarbeitung
                .layer(0, new LSTM.Builder()
                        .nIn(inputSize)
                        .nOut(preLayerSize1)
                        .activation(Activation.TANH)
                        .build())
                // Zweite versteckte LSTM-Schicht
                .layer(1, new LSTM.Builder()
                        .nIn(preLayerSize1)
                        .nOut(preLayerSize2)
                        .activation(Activation.TANH)
                        .build())
                // Hauptsächliche LSTM-Schicht für Kontext-Gedächtnis
                .layer(2, new LSTM.Builder()
                        .nIn(preLayerSize2)
                        .nOut(hiddenLayerSize)
                        .activation(Activation.TANH)
                        .build())
                // Zusätzliche LSTM-Nachverarbeitungsschicht
        ...
        logger.info("Verbessertes rekurrentes neuronales Netzwerk initialisiert mit Architektur: {} -> {} -> {} -> {} -> {} -> {}",
    ...
    public void train(String text, int epochs) {
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            for (int seqStart = 0; seqStart <= characters.length - timeSeriesLength - 1; seqStart++) {
                network.rnnClearPreviousState(); // Rücksetzen des internen Status für jede Sequenz
    ...
    public char predict(char input) {
        ...
        INDArray outputArray = network.output(inputArray, false);
